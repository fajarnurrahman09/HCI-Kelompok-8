        ...
        <section class="input-data">
            ...
            <div class="container">
                ...
                <div class="container">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr class="table-active">
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#sekremodaledit<?php echo $data['id'] ?>"><i class="fa fa-edit" aria-hidden="true"></i>edit</button>
                                        <a href="../buttondelete/DLTsekre.php?id=<?php echo $data['id']; ?>" onclick="return confirm('Apakah anda yakin ingin menghapus data ini ?')" class="btn btn-danger btn-sm"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                    </td>
                                </tr>
                                <!-- Modal Edit -->
                                <div class="modal fade" id="sekremodaledit<?php echo $data['id'] ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="sekremodaleditLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="sekremodaleditLabel">Edit</h5>
                                            ...
                                            <div class="modal-body">
                                                <form method="POST" enctype="multipart/form-data">
                                                    <?php
                                                    $id = $data['id'];
                                                    $sql_edit = $koneksi->query("select * from tabel_dsekre where id = '$id' ");
                            ...
                            if (isset($_POST['ubah'])) {
                                ...
                                if ($image != "") {
                                    ...
                                    if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
                                        ...
                                        $query = "UPDATE tabel_dsekre SET nama = '$nama', deskripsi = '$deskripsi', foto = '$nama_gambar_baru' WHERE id = '$id'";
                                        ...
                                        if (!$result) {
                                            ...
                                            echo "<script>alert('Data berhasil diedit.');window.location='sekre.php';</script>";
                                        }
                                    } else {
                                        //jika file ekstensi tidak jpg dan png maka alert ini yang tampil
                                        echo "<script>alert('Ekstensi gambar yang boleh hanya jpg atau png.');window.location='sekre.php';</script>";
                                    }
                                } else {
                                    $query = "UPDATE tabel_dsekre SET nama = '$nama', deskripsi = '$deskripsi' WHERE id = '$id'";
                                    $result = mysqli_query($koneksi, $query);
                                    // periska query apakah ada error
                                    if (!$result) {
                                        die("Query gagal dijalankan: " . mysqli_errno($koneksi) .
                    ...
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#sekremodal">Create</button>
                </div>

                <!-- Modal Create -->
                <div class="modal fade" id="sekremodal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="sekremodalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="sekremodalLabel">Divisi SARPRAS</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <form method="POST" enctype="multipart/form-data">
            ...
            if (isset($_POST['simpan'])) {
                ...
                if ($image != "") {
                    ...
                    if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
                        move_uploaded_file($file_tmp, 'gambar/' . $nama_gambar_baru); //memindah file gambar ke folder gambar
                        // jalankan query INSERT untuk menambah data ke database pastikan sesuai urutan (id tidak perlu karena dibikin otomatis)
                        $query = "INSERT INTO tabel_dsekre (nama, deskripsi, foto) VALUES ('$nama','$deskripsi','$nama_gambar_baru')";
                        $result = mysqli_query($koneksi, $query);
                        // periska query apakah ada error
                        if (!$result) {
                            die("Query gagal dijalankan: " . mysqli_errno($koneksi) .
                            ...
                            //tampil alert dan akan redirect ke halaman index.php
                            //silahkan ganti index.php sesuai halaman yang akan dituju
                            echo "<script>alert('Data berhasil ditambah.');window.location='sekre.php';</script>";
                        }
                    } else {
                        //jika file ekstensi tidak jpg dan png maka alert ini yang tampil
                        echo "<script>alert('Ekstensi gambar yang boleh hanya jpg atau png.');window.location='sekre.php';</script>";
                    ...
                    $query = "INSERT INTO tabel_dsekre (nama, deskripsi, foto) VALUES ('$nama', '$deskripsi', null)";
                    $result = mysqli_query($koneksi, $query);
                    // periska query apakah ada error
                    if (!$result) {
                        die("Query gagal dijalankan: " . mysqli_errno($koneksi) .
